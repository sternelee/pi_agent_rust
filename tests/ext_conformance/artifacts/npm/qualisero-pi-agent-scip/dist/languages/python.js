...
export class PythonAdapter {
    ...
    async installIndexer(projectRoot, options) {
        if (options?.confirm) {
            const acknowledged = await options.confirm('scip-python ships with pi-agent-scip via npm and is invoked automatically. No additional installation is required.');
            if (!acknowledged) {
